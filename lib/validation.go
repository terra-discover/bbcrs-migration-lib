...
// IsEmptyHtmlStrPtr
func IsEmptyHtmlStrPtr(str *string) (isEmpty bool) {
	if str == nil {
		isEmpty = true
		return
	}

	cleanHtmlTag := bluemonday.NewPolicy()
	strOnly := cleanHtmlTag.Sanitize(*str)

	removeSpace := strings.TrimSpace(strOnly)
...
// IsEmptyHtmlStr
func IsEmptyHtmlStr(str string) (isEmpty bool) {
	cleanHtmlTag := bluemonday.NewPolicy()
	strOnly := cleanHtmlTag.Sanitize(str)
